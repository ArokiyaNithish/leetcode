...
20            
21        return max_lengthView more 0/5Python3Auto12345678910class Solution:    def lengthOfLongestSubstring(self, s: str) -> int:        char_map = {}        start = 0        max_length = 0                for end in range(len(s)):            current_char = s[end]                        # If we've seen the character and it's inside the current window,SavedLn 1, Col 1You must run your code firstFindHeaderBarSizeFindTabBarSizeFindBorderBarSize
